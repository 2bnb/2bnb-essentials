...
class ACE_ZeusActions {
	class BNB_ES_Zeus_Tools {
		...
		class Get_Dead_Players {
			...
			statement = "[] call bnb_es_core_fnc_listDeadPlayers";

			class Get_Dead_West_Players {
				displayName = "List dead BLUFOR players";
				icon = "z\ace\addons\respawn\UI\Icon_Module_Rallypoint_ca.paa";
				statement = "[west] call bnb_es_core_fnc_listDeadPlayers";
				condition = "count ((call bnb_es_core_fnc_getDeadPlayers) select 0) > 0";
			};

			class Get_Dead_East_Players {
				displayName = "List dead OPFOR players";
				icon = "z\ace\addons\respawn\UI\Icon_Module_Rallypoint_ca.paa";
				statement = "[east] call bnb_es_core_fnc_listDeadPlayers";
				condition = "count ((call bnb_es_core_fnc_getDeadPlayers) select 1) > 0";
			};

			class Get_Dead_Resistance_Players {
				displayName = "List dead Independent players";
				icon = "z\ace\addons\respawn\UI\Icon_Module_Rallypoint_ca.paa";
				statement = "[resistance] call bnb_es_core_fnc_listDeadPlayers";
				condition = "count ((call bnb_es_core_fnc_getDeadPlayers) select 2) > 0";
			};

			class Get_Dead_Civilian_Players {
				displayName = "List dead Civilian players";
				icon = "z\ace\addons\respawn\UI\Icon_Module_Rallypoint_ca.paa";
				statement = "[civilian] call bnb_es_core_fnc_listDeadPlayers";
				condition = "count ((call bnb_es_core_fnc_getDeadPlayers) select 3) > 0";
			};
		};
		...
		class Set_Respawn_Timer {
			...
			statement = "[bnb_es_respawn_timer] call bnb_es_core_fnc_openTimerUI";
		};
	};
};
